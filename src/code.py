...
RGB_CYAN = (0, 255, 255)    # Cian/Celeste: Listo para Reinicio
...
P_WARN_LOW = 250.0  
P_RECOVERY = 220.0  # Usado como Umbral Crítico Mínimo de Presión para ESD
...
T_WARN_LOW = 120.0  
T_PREHEAT = 110.0   # Usado como Umbral Crítico Mínimo de Temperatura para ESD
...
def determine_system_color(estado_sistema, esd_active, esd_ready_to_reset, now):
    # Modo ESD
    if esd_active:
        if esd_ready_to_reset:
            # Color para Listo para Reinicio (Celeste/Cian)
            return RGB_CYAN  # Cian: listo para reinicio
        else:
...
while True:
    ...
    if esd_btn_state != esd_btn_state_last and not esd_btn_state: # Detección de flanco de bajada (pulsación)
        if not esd_active:
            esd_active = True
            esd_ready_to_reset = False
            print("[ALERTA] ESD activado manualmente.")
        elif esd_ready_to_reset:
            esd_active = False
            # Aseguramos que las salidas de seguridad y el láser estén OFF al resetear manualmente
            do_relief.value = False
            do_purga_a.value = False
            do_laser.value = False 
            print("[INFO] ESD reseteado.")
    esd_btn_state_last = esd_btn_state
    
    # Reiniciamos las variables de control en cada ciclo
    relief_activo = "No"
    purga_activa = "No"
    flow_state = "None"
    flow_led_on = False
    
    # Lógica para controlar las salidas de seguridad que apagan el láser
    # (Se actualizan más abajo en la lógica ESD si es necesario, pero reflejan el estado actual)
    relief_activo = "Si" if do_relief.value else "No"
    purga_activa = "Si" if do_purga_a.value else "No"
    ...
    if not esd_active:
        ...
        # Lógica para activar ESD automáticamente (Máximos y Mínimos)
        if (P_sim_kPa >= P_EMERG_HIGH or T_sim >= T_EMERG_HIGH or
            P_sim_kPa <= P_RECOVERY or T_sim <= T_PREHEAT):
            
            esd_active = True
            esd_ready_to_reset = False
            print("[ALERTA] Condición crítica (Mínimo/Máximo) detectada. Activando ESD automáticamente.")
    ...
    # ----------------- Lógica del modo ESD -----------------
        ...
        # El sistema está listo si ambas variables están dentro de su tolerancia inicial
        esd_ready_to_reset = (abs(P_sim_kPa - P_INITIAL) <= ESD_P_TOLERANCE and abs(T_sim - T_INITIAL) <= ESD_T_TOLERANCE)
        ...
        if esd_ready_to_reset:
            ...
            if current_P > P_INITIAL + ESD_P_TOLERANCE:
                do_relief.value = True
                relief_activo = "Si"
                MV_pct = 50.0
                P_sim_kPa -= (current_P - P_INITIAL) * dt * 0.2 
                estado_sistema = "ESD: Reduciendo Presión"
            elif current_P < P_INITIAL - ESD_P_TOLERANCE:
                do_relief.value = False
                relief_activo = "No"
                MV_pct = 0.0
                P_sim_kPa += (P_INITIAL - current_P) * dt * 0.2
            ...
            if current_T > T_INITIAL + ESD_T_TOLERANCE:
                do_purga_a.value = True
                purga_activa = "Si"
                SH_cmd = 50.0
                T_sim -= (current_T - T_INITIAL) * dt * 0.2
                estado_sistema = "ESD: Reduciendo Temperatura"
            elif current_T < T_INITIAL - ESD_T_TOLERANCE:
                do_purga_a.value = False
                purga_activa = "No"
                SH_cmd = 100.0
                T_sim += (T_INITIAL - current_T) * dt * 0.2
    ...
    # El láser se enciende si el alivio o la purga están activas
    do_laser.value = do_relief.value or do_purga_a.value
    
    rgb_color = determine_system_color(estado_sistema, esd_active, esd_ready_to_reset, now)
    ...
    if now - last_log > 0.1:
        output_data = "P:{:.1f},T:{:.1f},MV:{:.1f},SH:{:.1f},F:{},M:{},ESD:{},ESTADO:{},RELIEF:{},PURGE:{}".format(
            ...
            estado_sistema,
            # Se usan las variables de estado actualizadas para el log
            "Activado" if do_relief.value else "Desactivado",
            "Activado" if do_purga_a.value else "Desactivado"
        )
        print(output_data)
        last_log = now
